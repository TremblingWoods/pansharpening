...
class CrossmModalityTransformerBlock(nn.Module):
    def __init__(self, dim, out_dim, num_heads, window_size=1, attn_window_size=None, shift_size=0, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # else:
        #     self.pos = nn.Conv2d(dim, dim, window_size//2*2+1, 1, window_size//2, groups=dim, bias=True)
        self.crossmodality = Crossmodality()
    ...
    def forward(self, pan, lms):
        pan = self.crossmodality(pan,lms)
        pan = pan + self.pos(pan)
        lms = lms + self.pos(lms)
        attn_out = self.attn(self.norm1_pan(pan),self.norm1_lms(lms))
        pan = pan + self.drop_path(attn_out)
        pan = pan + self.drop_path(self.mlp(self.norm3(pan)))
...
class Crossmodality(nn.Module):
    def __init__(self):
        ...
        # 在构造函数中定义神经网络的层次结构
        num_channels = 32
        self.conv_1 = nn.Conv2d(num_channels, num_channels, kernel_size=1, padding=0, bias=True)
        self.act_1 = nn.GELU()
        self.conv_2 = nn.Conv2d(num_channels*2, num_channels, kernel_size=1, padding=0, bias=True)
        self.act_2 = nn.GELU()
        self.conv_3 = nn.Conv2d(num_channels, 1, kernel_size=1, padding=0, bias=True)
...
class Net(nn.Module):
    def __init__(self, num_channels=4,channels=None,base_filter=None,args=None):
        ...
        self.encoderlms1 = HinResBlock(channels,channels)
        self.CMTB1 = CrossmModalityTransformerBlock(dim=channels, out_dim=channels, num_heads=8, window_size=8,
                                                    ...
                                                    img_size=(128, 128))
        self.CMTB1_2 = CrossmModalityTransformerBlock(dim=channels, out_dim=channels, num_heads=8, window_size=8,
                                                      attn_window_size=None, shift_size=0, mlp_ratio=4., qkv_bias=True,
                                                      qk_scale=None, drop=0., attn_drop=0.,
                                                      ...
                                                      deformable=False, learnable=True, restart_regression=None,
                                                      img_size=(128, 128))
        self.encoderpan1 = HinResBlock(channels, channels)
        self.encoderlms2 = HinResBlock(channels, channels)
        self.CMTB2 = CrossmModalityTransformerBlock(dim=channels, out_dim=channels, num_heads=8, window_size=8,
                                                    attn_window_size=None, shift_size=0, mlp_ratio=4., qkv_bias=True,
                                                    qk_scale=None, drop=0., attn_drop=0.,
                                                    drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm, stride=False,
                                                    relative_pos_embedding=False, deform_relative_pos_embedding=False,
                                                    deformable=False, learnable=True, restart_regression=None,
                                                    img_size=(128, 128))
        self.CMTB2_2 = CrossmModalityTransformerBlock(dim=channels, out_dim=channels, num_heads=8, window_size=8,
                                                    attn_window_size=None, shift_size=0, mlp_ratio=4., qkv_bias=True,
                                                    qk_scale=None, drop=0., attn_drop=0.,
                                                    drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm, stride=False,
                                                    relative_pos_embedding=False, deform_relative_pos_embedding=False,
        ...
        self.encoderlms3 = HinResBlock(channels, channels)
        self.CMTB3 = CrossmModalityTransformerBlock(dim=channels, out_dim=channels, num_heads=8, window_size=8,
                                                    attn_window_size=None, shift_size=0, mlp_ratio=4., qkv_bias=True,
                                                    qk_scale=None, drop=0., attn_drop=0.,
                                                    drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm, stride=False,
                                                    relative_pos_embedding=False, deform_relative_pos_embedding=False,
                                                    deformable=False, learnable=True, restart_regression=None,
                                                    img_size=(128, 128))
        self.CMTB3_2 = CrossmModalityTransformerBlock(dim=channels, out_dim=channels, num_heads=8, window_size=8,
                                                    attn_window_size=None, shift_size=0, mlp_ratio=4., qkv_bias=True,
                                                    qk_scale=None, drop=0., attn_drop=0.,
                                                    drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm, stride=False,
                                                    relative_pos_embedding=False, deform_relative_pos_embedding=False,
        ...
        self.fuse = nn.Conv2d(channels * 2, channels, kernel_size=1, padding=0, bias=True)
    ...
    def forward(self, ms,_,pan):
        ...
        CMTB2_out = self.CMTB2(panf, msf)
        CMTB22_out = self.CMTB2_2(msf, panf)
        panf = panf + CMTB2_out
        msf = msf + CMTB22_out

        msf = self.encoderlms3(msf)
        panf = self.encoderpan3(panf)
        CMTB3_out = self.CMTB3(panf, msf)
        CMTB32_out = self.CMTB3_2(msf, panf)
        panf = panf + CMTB3_out
        msf = msf + CMTB32_out

        HR = self.conv_out(self.fuse(torch.cat((panf,msf), dim=1)))
        return HR +ms
