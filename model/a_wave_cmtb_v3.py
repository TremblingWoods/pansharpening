...
class WaveletEnhancedAttentionBlock(nn.Module):
    def __init__(self, channels, wmsa_module):
        ...
        # Patch Embedding 层，用于对 DWT 输出的低频分量进行嵌入
        self.patch_embed = nn.Conv2d(channels, channels, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        # 1.5 Patch Embedding：对低频分量 X_LL 进行进一步的特征嵌入
        X_LL = self.patch_embed(X_LL)
...
class WEVT(nn.Module):
    def __init__(self, dim, out_dim, num_heads, window_size=1, attn_window_size=None, shift_size=0, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0.1, attn_drop=0.1,
                 drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm, stride=False, relative_pos_embedding=False, deform_relative_pos_embedding=False,
                 deformable=False, learnable=True, restart_regression=None, img_size=(1,1), channel = None):
        super().__init__()
        # if shift:
        #     self.shift_size = window_size // 2
        # else:
        ...
        #小波变化：
        wmsa_module = DeformableWindowAttention(dim=channel, num_heads=num_heads, window_size=window_size, img_size=img_size)
        self.wab = WaveletEnhancedAttentionBlock(channels=channel, wmsa_module=wmsa_module)

        # 用 1x1 卷积融合重构后的特征
        self._conv = nn.Conv2d(channel, channel, kernel_size=1)
    ...
    def forward(self, pan, lms):
        ...
        #对结果分别进行小波变化
        pan_ = self.wab(self.norm1_pan(pan))
        lms_ = self.wab(self.norm1_pan(lms))

        # print(attn_out)
        pan = pan + pan_  #线性相加不太好，用个卷积
        pan = self._conv(pan)

        pan_atten = pan + self.drop_path(self.mlp(self.norm3(pan)))
        pan_atten = self._conv(pan_atten)

        lms = lms+ lms_
        lms = self._conv(lms)

        lms_atten = lms + self.drop_path_2(self.mlp(self.norm3_2(lms)))
        lms_atten = self._conv(lms_atten)
...
class Net(nn.Module):
    def __init__(self, num_channels=4,channels=None,base_filter=None,args=None):
        ...
        #参数还没调
        self.WEVT1 = WEVT(dim=channels, out_dim=channels, num_heads=8, window_size=8,
                                                    attn_window_size=None, shift_size=0, mlp_ratio=2., qkv_bias=True,
                                                    qk_scale=None, drop=0., attn_drop=0.0,
                                                    drop_path=0.1, act_layer=nn.GELU, norm_layer=LayerNorm, stride=False,
                                                    relative_pos_embedding=False, deform_relative_pos_embedding=False,
                                                    deformable=False, learnable=True, restart_regression=None,
                                                    img_size=(128, 128),channel=channels)

        self.WEVT2 = WEVT(dim=2*channels, out_dim=2*channels, num_heads=8, window_size=8,
                                                    attn_window_size=None, shift_size=0, mlp_ratio=2., qkv_bias=True,
                                                    qk_scale=None, drop=0., attn_drop=0.0,
                                                    drop_path=0.1, act_layer=nn.GELU, norm_layer=LayerNorm, stride=False,
                                                    relative_pos_embedding=False, deform_relative_pos_embedding=False,
                                                    deformable=False, learnable=True, restart_regression=None,
                                                    img_size=(64, 64),channel=2*channels)

        self.WEVT3 = WEVT(dim=4*channels, out_dim=4*channels, num_heads=8, window_size=8,
                                                    attn_window_size=None, shift_size=0, mlp_ratio=2., qkv_bias=True,
                                                    qk_scale=None, drop=0., attn_drop=0.0,
                                                    drop_path=0.1, act_layer=nn.GELU, norm_layer=LayerNorm, stride=False,
                                                    relative_pos_embedding=False, deform_relative_pos_embedding=False,
                                                    deformable=False, learnable=True, restart_regression=None,
                                                    img_size=(32, 32),channel=4*channels)
    ...
    def forward(self, ms,_,pan):
        ...
        pan_attn , lms_attn = self.CMTB1(panf, msf)#size 4  [N,32,H,W]

        pan_1,ms_1 = pan_attn,lms_attn #size 4
        #对每层得到的pan1和ms1分别进行一个小波变化增强效果，增强高频和低频信息，WEVT层
        pan_1,ms_1 = self.WEVT1(pan_1,ms_1)  #size 4  [N,32,H,W]


        pan_attn = self.patition1(pan_attn)
        lms_attn = self.patition1_2(lms_attn)
        pan_attn, lms_attn = self.CMTB2(pan_attn, lms_attn)#[N,64,H/2,W/2]
        pan_2,ms_2 = pan_attn,lms_attn #size 2
        #对每层得到的pan2和ms3分别进行一个小波变化增强效果，增强高频和低频信息，WEVT层
        pan_2,ms_2 = self.WEVT2(pan_2,ms_2)


        pan_attn = self.patition2(pan_attn)
        lms_attn = self.patition2_2(lms_attn)

        panf, msf = self.CMTB3(pan_attn, lms_attn)#[N,128,H/4,W/4]
        pan_3,ms_3 = panf,msf #size1
        #对每层得到的pan1和ms1分别进行一个小波变化增强效果，增强高频和低频信息，WEVT层
        pan_3, ms_3 = self.WEVT3(pan_3, ms_3)
