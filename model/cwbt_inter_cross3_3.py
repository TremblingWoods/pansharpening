...
class AttentionTransformerBlock(nn.Module):
    def __init__(self, dim, out_dim, num_heads, window_size=1, attn_window_size=None, shift_size=0, mlp_ratio=2., qkv_bias=True, qk_scale=None, drop=0.1, attn_drop=0.078,
    ...
    def forward(self, pan, lms):
        ...
        # print('attn_out',attn_out[0].shape)
        # print(attn_out)
        pan = pan + self.drop_path(attn_out[0])
        # print('CMTB2',pan.shape)
        a = self.drop_path(self.mlp(self.norm3(pan)))
        # print('a',a.shape)
        pan_atten = pan + a
...
class Net(nn.Module):
    def __init__(self, num_channels=4,channels=None,base_filter=None,args=None):
        ...
        self.crossmodel1 = Crossmodality(num_channels)
        self.crossmodel2 = Crossmodality(num_channels)
        ...
        self.CMTB2 = AttentionTransformerBlock(dim=channels*2, out_dim=channels*2, num_heads=8, window_size=8,
                                                    attn_window_size=None, shift_size=0, mlp_ratio=2., qkv_bias=True,
                                                    qk_scale=None, drop=0., attn_drop=0.1,
                                                    drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm, stride=False,
                                                    relative_pos_embedding=False, deform_relative_pos_embedding=False,
                                                    deformable=False, learnable=True, restart_regression=None,
                                                    img_size=(64, 64))
        self.patition2 = PatchMerging(channels*2,2*channels)
        self.patition2_2 = PatchMerging(channels*2,2*channels)
        # self.crossmodel3 = Crossmodality()
        self.encoderpan3 = HinResBlock(channels*2, channels*2)
        self.encoderlms3 = HinResBlock(channels*2, channels*2)
        self.CMTB3 = AttentionTransformerBlock(dim=channels*2, out_dim=channels*2, num_heads=8, window_size=8,
                                                    attn_window_size=None, shift_size=0, mlp_ratio=2., qkv_bias=True,
                                                    qk_scale=None, drop=0., attn_drop=0.1,
                                                    drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm, stride=False,
                                                    relative_pos_embedding=False, deform_relative_pos_embedding=False,
        ...
        self.fuse = nn.Conv2d(channels //4, channels//4, kernel_size=1, padding=0, bias=True)
        self.relu = nn.LeakyReLU()
        self.conv_out = Refine(channels//4, 4)
    ...
    def forward(self, ms,_,pan):
        ...
        msf_attn = self.encoderlms2(lms_attn)
        pan_attn = self.encoderpan2(pan_attn)
        pan_attn = self.patition1(pan_attn)
        lms_attn = self.patition1_2(lms_attn)
        pan_attn, lms_attn = self.CMTB2(pan_attn, lms_attn)

        msf_attn = self.encoderlms3(lms_attn)
        pan_attn = self.encoderpan3(pan_attn)
