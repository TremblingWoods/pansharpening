...
class Crossmodality(nn.Module):
    def __init__(self):
        ...
        # 在构造函数中定义神经网络的层次结构
        num_channels = 32
        self.conv_1 = nn.Conv2d(num_channels, num_channels*2, kernel_size=3, padding=1, bias=True)
        self.conpan_1 = nn.Conv2d(num_channels, num_channels * 2, kernel_size=3, padding=1, bias=True)
        self.act_1 = nn.GELU()
        self.conv_2 = nn.Conv2d(num_channels*4, num_channels, kernel_size=3, padding=1, bias=True)
        self.act_2 = nn.GELU()
        self.conv_3 = nn.Conv2d(num_channels, 1, kernel_size=3, padding=1, bias=True)
        self.conv_4 = nn.Conv2d(num_channels, 1, kernel_size=3, padding=1, bias=True)
    ...
    def forward(self, pan, ms):
        ...
        pan_conv1 = self.conpan_1(pan)
        x_h1_cat = torch.cat((lms_conv, pan_conv1), dim=1)
...
class AttentionTransformerBlock(nn.Module):
    ...
    def forward(self, pan, lms):
        ...
        lms = lms+ self.drop_path(attn_out)
        lms_atten = lms + self.drop_path(self.mlp(self.norm3(pan)))
        return lms_atten
...
class Net(nn.Module):
    def __init__(self, num_channels=4,channels=None,base_filter=None,args=None):
        ...
        self.CMTB2 = AttentionTransformerBlock(dim=channels, out_dim=channels, num_heads=8, window_size=8,
                                                    attn_window_size=None, shift_size=0, mlp_ratio=4., qkv_bias=True,
                                                    qk_scale=None, drop=0., attn_drop=0.1,
                                                    drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm, stride=False,
                                                    relative_pos_embedding=False, deform_relative_pos_embedding=False,
        ...
        self.CMTB3 = AttentionTransformerBlock(dim=channels, out_dim=channels, num_heads=8, window_size=8,
                                                    attn_window_size=None, shift_size=0, mlp_ratio=4., qkv_bias=True,
                                                    qk_scale=None, drop=0., attn_drop=0.1,
                                                    drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm, stride=False,
                                                    relative_pos_embedding=False, deform_relative_pos_embedding=False,
        ...
        self.fuse = nn.Conv2d(channels * 2, channels, kernel_size=3, padding=1, bias=True)
    ...
    def forward(self, ms,_,pan):
        ...
        panf = self.crossmodel1(panf,msf)
        ...
        panf = self.crossmodel3(panf, msf)
