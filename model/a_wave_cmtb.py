...
class AttentionTransformerBlock_wave(nn.Module):
    def __init__(self, dim, out_dim, num_heads, window_size=1, attn_window_size=None, shift_size=0, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0.1, attn_drop=0.1,
                 drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm, stride=False, relative_pos_embedding=False, deform_relative_pos_embedding=False,
                 deformable=False, learnable=True, restart_regression=None, img_size=(1,1)):
        super().__init__()
...
class Net(nn.Module):
    def __init__(self, num_channels=4,channels=None,base_filter=None,args=None):
        ...
        self.wavef2 = WInvBlock(4 * channels, 2 * channels)
        self.hfb2 = DwtFusionMoudle(2*channels)

        self.wavef3 = WInvBlock(8 * channels, 4*channels)
        self.hfb3 = DwtFusionMoudle(4*channels)

        self.wop1 = HinResBlock(channels, channels)
        self.wop2 = HinResBlock(2*channels, 2*channels)
        self.wop3 = HinResBlock(4*channels, 4*channels)
    ...
    def forward(self, ms,_,pan):
        ...
        panf = self.crossmodel2(panf,msf) #  [N,32,H,W]  panf和msf

        #在进可变窗口之前先进行一次小波变化：
        msf = self.wavef1(torch.cat([msf, panf], 1)) #[N,32,H,W]
        msf = self.hfb1(msf,panf) #[N,32,H,W]
        panf = self.wop1(panf)  #[N,32,H,W]

        #第一次cmtb
        pan_attn , lms_attn = self.CMTB1(panf, msf)
        pan_1,ms_1 = pan_attn,lms_attn #size 4  [N,32,H,W]

        pan_attn = self.patition1(pan_attn)  # [N,32,H,W]->[N,64,H/2,W/2]
        lms_attn = self.patition1_2(lms_attn)  # [N,32,H,W]->[N,64,H/2,W/2]

        #第二次小波变化
        lms_attn = self.wavef2(torch.cat([lms_attn, pan_attn], 1)) #[N,64,H/2,W/2]
        lms_attn = self.hfb2(lms_attn, pan_attn)  #[N,64,H/2,W/2]
        pan_attn = self.wop2(pan_attn)  #[N,64,H/2,W/2]

        # 第二次cmtb
        pan_attn, lms_attn = self.CMTB2(pan_attn, lms_attn) #[N,64,H/2,W/2]
        pan_2,ms_2 = pan_attn,lms_attn #size 2 [N,64,H/2,W/2]

        pan_attn = self.patition2(pan_attn)  # [N,64,H/2,W/2]-> [N,256,H/4,W/4]
        lms_attn = self.patition2_2(lms_attn) # [N,64,H/2,W/2]-> [N,256,H/4,W/4]

        #第三次小波变化
        lms_attn = self.wavef3(torch.cat([lms_attn, pan_attn], 1)) #[N,256,H/4,W/4]
        lms_attn = self.hfb3(lms_attn, pan_attn)  #[N,256,H/4,W/4]
        pan_attn = self.wop3(pan_attn)  #[N,256,H/4,W/4]

        # 第三次cmtb
        panf, msf = self.CMTB3(pan_attn, lms_attn) # 这个地方的输入256通道，可能上面的通道搞错了，不管了先训练着
